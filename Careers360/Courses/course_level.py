...
for id in obj:
    ...
    if(Counting >= start_point):
        ...
        # print(url_req)
        # driver.get(url_req)
        course_level_dict = obj[id]['course_level']

        try:
            for k, v in zip(course_level_dict.keys(), course_level_dict.values()):
                # clear D1
                D1 = {}
                # print(k, v)
                url_req = str(v)
                ...
                try:
                    ...
                    if(total_c_int > 30):
                        for page in range(1, int(total_c_int/30)+2):
                            try:
                                # s[:int(s.find('?')+1)] + "page=1&" + s[s.find('?')+1:]
                                url_req_ = url_req[:url_req.find(
                                    '?')+1] + str("page=" + str(page)) + '&' + url_req[url_req.find('?')+1:]
                                print(url_req_)
                                driver.get(url_req_)
                                for course_name in driver.find_elements(By.CLASS_NAME, 'heading4'):
                                    D1[course_name.find_element(By.TAG_NAME, 'a').text] = course_name.find_element(
...
with open('D:\\TestOne\\College360\\Courses\\all_course_level_data.json', "r") as f:
    data_file = json.load(f)
data_file.update(D3)
json_object = json.dumps(data_file, indent=4)

# Writing with new dict objects
with open("D:\\TestOne\\College360\\Courses\\all_course_level_data.json", "w") as f:
