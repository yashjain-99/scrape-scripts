...
for id in obj:
    ...
    if(Counting >= start_point):
        ...
        # print(url_req)
        # driver.get(url_req)
        study_mode_dict = obj[id]['study_mode']

        # second method if it is not working
        # s_keys = study_mode_dict.keys()
        # if('Distance' in s_keys):
        #     study_mode_dict.update(
        #         {'Distance': str(str('/courses?study_mode=3'))})
        # if('Full time' in s_keys):
        #     study_mode_dict.update(
        #         {'Full time': str(str('/courses?study_mode=2'))})
        # if('Part time' in s_keys):
        #     study_mode_dict.update(
        #         {'Part time': str(str('/courses?study_mode=1'))})
        # if('Online' in s_keys):
        #     study_mode_dict.update(
        #         {'Online': str(str('/courses?study_mode=4'))})

        try:
            for k, v in zip(study_mode_dict.keys(), study_mode_dict.values()):
                # clear D1
                D1 = {}
                # print(k, v)
                url_req = str(v)
...
with open('D:\\TestOne\\College360\\Courses\\all_study_mode_data.json', "r") as f:
    data_file = json.load(f)
data_file.update(D3)
json_object = json.dumps(data_file, indent=4)

# Writing with new dict objects
with open("D:\\TestOne\\College360\\Courses\\all_study_mode_data.json", "w") as f:
