...
for id in obj:
    ...
    if(Counting >= start_point):
        ...
        # print(url_req)
        # driver.get(url_req)
        stream_dict = obj[id]['stream']

        try:
            for k, v in zip(stream_dict.keys(), stream_dict.values()):
                # clear D1
                D1 = {}
                # print(k, v)
...
with open('D:\\TestOne\\College360\\Courses\\all_stream_data.json', "r") as f:
    data_file = json.load(f)
data_file.update(D3)
json_object = json.dumps(data_file, indent=4)

# Writing with new dict objects
with open("D:\\TestOne\\College360\\Courses\\all_stream_data.json", "w") as f:
