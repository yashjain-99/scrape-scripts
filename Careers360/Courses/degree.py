...
for id in obj:
    ...
    if(Counting >= start_point):
        ...
        # print(url_req)
        # driver.get(url_req)
        degree_dict = obj[id]['degree']

        try:
            for k, v in zip(degree_dict.keys(), degree_dict.values()):
                # clear D1
                D1 = {}
                # print(k, v)
                url_req = str(clg_link + v)
                driver.get(url_req)
...
with open('D:\\TestOne\\College360\\Courses\\all_degree_data.json', "r") as f:
    data_file = json.load(f)
data_file.update(D3)
json_object = json.dumps(data_file, indent=4)

# Writing with new dict objects
with open("D:\\TestOne\\College360\\Courses\\all_degree_data.json", "w") as f:
